...
DEVS = [
    ...
    {
        "name": "Frontend — React SPA (“Relay”)",
        "scope": "The custom admin panel (D-107): React SPA against the Django/DRF API, "
                 "JWT auth. Owner: Dev 3 (React) + Dev 1 (flow-builder logic). Each screen "
                 "is gated on its DRF endpoint existing. The live phone preview is Phase 2. "
                 "NOTE: the former 'Dev 3 admin CRUD' tickets become DRF API endpoints "
                 "feeding these screens.",
        "epics": [
            ("Epic FE-A — Scaffold & foundation", [
                dict(id="F-01", title="React app scaffold",
                     tag="S", size="M", depends="—", ai="Yes", refs="frontend_spec.md §2",
                     desc="Stand up the SPA with routing and the Relay design tokens.",
                     notes=["Vite + React + TypeScript; routing; tokens as CSS vars; brand fonts."],
                     accept=["App builds and serves; tokens + fonts applied."]),
                dict(id="F-02", title="API client + auth handling",
                     tag="S", size="M", depends="F-01", ai="Partial", refs="architecture.md §6.1",
                     desc="Central API wrapper with JWT and 401 handling.",
                     notes=["Bearer token on every call; store/clear token; 401 → login; base URL from env."],
                     accept=["Authed calls succeed; 401 redirects to login."]),
                dict(id="F-03", title="App shell",
                     tag="S", size="M", depends="F-01", ai="Yes", refs="frontend_spec.md §3",
                     desc="Persistent top bar + sidebar + protected routes.",
                     notes=["Top bar (logo, breadcrumb, account); sidebar (Dashboard, Clients); route guard."],
                     accept=["Authed users see the shell; unauth redirected."]),
                dict(id="F-04", title="UI primitives",
                     tag="S", size="L", depends="F-01", ai="Yes", refs="frontend_spec.md §2/§10",
                     desc="Reusable components matching the design system.",
                     notes=["Button, Card, Badge, Table, Modal, Toast, Input, Toggle, Skeleton, EmptyState."],
                     accept=["Primitives render per tokens and are reused across screens."]),
            ]),
            ("Epic FE-B — Auth & dashboard", [
                dict(id="F-05", title="Login screen",
                     tag="P", size="M", depends="F-02,F-04", ai="Yes", refs="FR-01",
                     desc="Single-account sign-in.",
                     notes=["Email+password; error/loading states; success → Dashboard."],
                     accept=["Valid login lands on Dashboard; bad creds show inline error."]),
                dict(id="F-06", title="Dashboard",
                     tag="P", size="L", depends="F-03,F-04", ai="Partial", refs="frontend_spec.md §5",
                     desc="Overview with stats and client cards.",
                     notes=["4 stat tiles; how-it-works; clickable client cards; empty state."],
                     accept=["Stats render; clicking a card opens client detail."]),
            ]),
            ("Epic FE-C — Clients & config", [
                dict(id="F-07", title="Clients list",
                     tag="P", size="M", depends="F-04", ai="Yes", refs="FR-15",
                     desc="Table of all tenants.",
                     notes=["Columns per spec; row click → detail; empty state; New client button."],
                     accept=["Clients listed; row opens detail."]),
                dict(id="F-08", title="Create client modal",
                     tag="P", size="M", depends="F-07", ai="Yes", refs="FR-16",
                     desc="Create a tenant from the list.",
                     notes=["Fields + validation (name ≥2 enables Create); toast; navigate to detail."],
                     accept=["New client created and opened in Flow builder tab."]),
                dict(id="F-09", title="Client detail shell + tabs",
                     tag="P", size="M", depends="F-03", ai="Yes", refs="frontend_spec.md §3",
                     desc="Detail header + the three tabs.",
                     notes=["Header (emoji, name, status, back); Settings/Flow builder/Conversations."],
                     accept=["Tabs switch; defaults to Flow builder."]),
                dict(id="F-10", title="Settings tab",
                     tag="P", size="L", depends="F-09,F-04", ai="Partial", refs="FR-05/11/12",
                     desc="Per-client configuration.",
                     notes=["Greeting/closing; handoff toggle reveals email (+warning); channels & identity."],
                     accept=["Settings save and reflect on reload."]),
            ]),
            ("Epic FE-D — Flow builder (hardest)", [
                dict(id="F-11", title="Step list (collapsed)",
                     tag="D", size="L", depends="F-09", ai="Partial", refs="FR-17 / spec §8a",
                     desc="Browse-mode list of step cards.",
                     notes=["Mono ID, label, START/END badge, option count, option map; Add step."],
                     accept=["Steps render with their option maps."]),
                dict(id="F-12", title="Step editor (expanded)",
                     tag="D", size="L", depends="F-11", ai="NO (core UX)", refs="spec §8b",
                     desc="Edit one step inline.",
                     notes=["Name, message (char count), single-start toggle, option rows (20/20 + target), delete."],
                     accept=["Editing a step updates the collapsed card live."]),
                dict(id="F-13", title="Live validation + activation",
                     tag="D", size="L", depends="F-12", ai="NO (correctness)", refs="spec §8d",
                     desc="Client-side mirror of the validation gate.",
                     notes=["V-01..V-06 on every change; banner; Activate disabled when invalid."],
                     accept=["Invalid flow can't activate; errors clear when fixed. Server is source of truth."]),
            ]),
            ("Epic FE-E — Conversations", [
                dict(id="F-14", title="Conversation list",
                     tag="P", size="M", depends="F-09", ai="Yes", refs="FR-18",
                     desc="Read-only session log.",
                     notes=["Session rows: channel badge, customer id, path trail, timestamp, status; newest first."],
                     accept=["Sessions listed with correct status badges."]),
                dict(id="F-15", title="Message thread expand",
                     tag="D", size="M", depends="F-14", ai="Yes", refs="spec §9",
                     desc="Inline thread on row expand.",
                     notes=["Bot left, customer right, system events centred; toggle open/closed."],
                     accept=["Expanding a row shows its full message thread."]),
            ]),
            ("Epic FE-F — Integration & polish", [
                dict(id="F-16", title="Global states",
                     tag="P", size="M", depends="F-04", ai="Yes", refs="spec §10",
                     desc="Consistent feedback patterns.",
                     notes=["Toasts, loading skeletons, empty states applied everywhere."],
                     accept=["Every list/async action has proper state feedback."]),
                dict(id="F-17", title="Wire all screens to live API",
                     tag="D", size="L", depends="F-05..F-16", ai="Partial", refs="spec §12",
                     desc="Replace mock data with real endpoints.",
                     notes=["Every screen reads/writes real API; error + loading handled."],
                     accept=["No mock data remains; full flow works end-to-end."]),
                dict(id="F-P1", title="Live phone preview / simulator (Phase 2)",
                     tag="D", size="L", depends="F-13", ai="NO", refs="spec §11",
                     desc="Deferred: in-browser WhatsApp/IG simulator running the real flow.",
                     notes=["Channel toggle + reset; branch/re-prompt/handoff. NOT in v1."],
                     accept=["Phase 2 — simulator runs a client's real flow in-browser."]),
            ]),
        ],
    },
]
